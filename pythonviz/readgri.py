...
def readgri(fname):
    ...
    for i in range(NB):
        ...
        B.append(Bi)
        if (Bname[len(Bname)-1]) == "Curve1":
            B_upper.append(Bi)
        elif (Bname[len(Bname)-1]) == "Curve5":
            B_lower.append(Bi)
    ...
    IE, BE = edgehash(E, B)
    temp, B_upper = edgehash(E,B_upper)
    temp, B_lower = edgehash(E,B_lower)
    Mesh = {'V':V, 'E':E, 'IE':IE, 'BE':BE, 'Bname':Bname, 'B_up':B_upper, 'B_low':B_lower}
    return Mesh
